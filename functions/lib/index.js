...
exports.depositWebhook = exports.createAdCampaign = exports.boostPost = exports.sendStarGift = exports.purchaseStars = exports.supportCreator = exports.withdrawETB = exports.depositETB = void 0;
 ...
 */
function getOrCreateWallet(walletSnap, userId) {
 ...
 */
exports.depositETB = (0, https_1.onCall)({ cors: true }, async (request) => {
    const userId = request.auth?.uid;
    if (!userId) {
        throw new https_1.HttpsError("unauthenticated", "Authentication required to deposit funds.");
    }
    const rawAmount = request.data.amountETB;
    const txRef = request.data.txRef;
    if (!rawAmount || typeof rawAmount !== "number" || rawAmount <= 0) {
        throw new https_1.HttpsError("invalid-argument", "A positive deposit amount in ETB is required.");
    }
    const depositAmount = roundCurrency(rawAmount);
    if (depositAmount < 5) {
        throw new https_1.HttpsError("invalid-argument", "Minimum deposit is 5.00 ETB.");
    }
 ...
 */
exports.withdrawETB = (0, https_1.onCall)({ cors: true }, async (request) => {
    const userId = request.auth?.uid;
    if (!userId) {
        throw new https_1.HttpsError("unauthenticated", "Authentication required to withdraw funds.");
    }
    const rawWithdraw = request.data.withdrawAmountETB;
    if (!rawWithdraw || typeof rawWithdraw !== "number" || rawWithdraw <= 0) {
        throw new https_1.HttpsError("invalid-argument", "A valid withdrawal amount is required.");
    }
    const withdrawAmount = roundCurrency(rawWithdraw);
    if (withdrawAmount < 50) {
        throw new https_1.HttpsError("invalid-argument", "Minimum withdrawal is 50.00 ETB.");
    }
    const { accountNumber, accountHolderName, bankCode } = request.data;
    if (!accountNumber || !accountHolderName) {
        throw new https_1.HttpsError("invalid-argument", "Valid bank account details are required.");
    }
    const walletRef = db.collection("wallets").doc(userId);
    const txRefDoc = db.collection("transactions").doc();
    return await db.runTransaction(async (transaction) => {
        const walletSnap = await transaction.get(walletRef);
        if (!walletSnap.exists) {
            throw new https_1.HttpsError("failed-precondition", "Wallet not initialized or zero balance.");
        }
        const wallet = getOrCreateWallet(walletSnap, userId);
        // Strict Live Balance Audit Check
        if (wallet.chapaBalance < withdrawAmount) {
            throw new https_1.HttpsError("failed-precondition", `Insufficient funds. Available: ${wallet.chapaBalance.toFixed(2)} ETB, Requested: ${withdrawAmount.toFixed(2)} ETB.`);
        }
 ...
 */
exports.supportCreator = (0, https_1.onCall)({ cors: true }, async (request) => {
    const senderId = request.auth?.uid;
    if (!senderId) {
        throw new https_1.HttpsError("unauthenticated", "Authentication required to support creator.");
    }
    const { creatorId, amountETB, message } = request.data;
    if (!creatorId || typeof creatorId !== "string") {
        throw new https_1.HttpsError("invalid-argument", "Creator ID is required.");
    }
    if (creatorId === senderId) {
        throw new https_1.HttpsError("invalid-argument", "Cannot send a tip to your own wallet.");
    }
    if (!amountETB || typeof amountETB !== "number" || amountETB <= 0) {
        throw new https_1.HttpsError("invalid-argument", "A valid tip amount is required.");
    }
    ...
    return await db.runTransaction(async (transaction) => {
        ...
        if (!senderSnap.exists) {
            throw new https_1.HttpsError("failed-precondition", "Sender wallet not found.");
        }
        const senderWallet = getOrCreateWallet(senderSnap, senderId);
        if (senderWallet.chapaBalance < tipAmount) {
            throw new https_1.HttpsError("failed-precondition", `Insufficient funds. Current balance: ${senderWallet.chapaBalance.toFixed(2)} ETB, Tip amount: ${tipAmount.toFixed(2)} ETB.`);
        }
        ...
        transaction.set(creatorAnalyticsRef, {
 ...
 */
exports.purchaseStars = (0, https_1.onCall)({ cors: true }, async (request) => {
    const userId = request.auth?.uid;
    if (!userId) {
        throw new https_1.HttpsError("unauthenticated", "Authentication required to buy stars.");
    }
    const starCount = Math.floor(Number(request.data.totalStars));
    if (!starCount || starCount <= 0) {
        throw new https_1.HttpsError("invalid-argument", "Positive integer star count required.");
    }
    ...
    return await db.runTransaction(async (transaction) => {
        const walletSnap = await transaction.get(walletRef);
        if (!walletSnap.exists) {
            throw new https_1.HttpsError("failed-precondition", "Wallet not found.");
        }
        const wallet = getOrCreateWallet(walletSnap, userId);
        if (wallet.chapaBalance < totalCost) {
            throw new https_1.HttpsError("failed-precondition", `Insufficient funds to purchase ${starCount} stars. Cost: ${totalCost.toFixed(2)} ETB, Balance: ${wallet.chapaBalance.toFixed(2)} ETB.`);
        }
        const now = admin.firestore.Timestamp.now();
 ...
 */
exports.sendStarGift = (0, https_1.onCall)({ cors: true }, async (request) => {
    const senderId = request.auth?.uid;
    if (!senderId) {
        throw new https_1.HttpsError("unauthenticated", "Authentication required to gift stars.");
    }
    ...
    if (creatorId === senderId) {
        throw new https_1.HttpsError("invalid-argument", "Cannot send a star gift to yourself.");
    }
    ...
    return await db.runTransaction(async (transaction) => {
        ...
        if (senderWallet.virtualStars < giftCount) {
            throw new https_1.HttpsError("failed-precondition", `Insufficient stars. Available: ${senderWallet.virtualStars}, Required: ${giftCount}.`);
        }
 ...
 */
exports.boostPost = (0, https_1.onCall)({ cors: true }, async (request) => {
    const userId = request.auth?.uid;
    if (!userId) {
        throw new https_1.HttpsError("unauthenticated", "Authentication required to boost post.");
    }
    ...
    if (dailyBudget < 10) {
        throw new https_1.HttpsError("invalid-argument", "Minimum daily budget for boost is 10.00 ETB.");
    }
    if (duration < 1 || duration > 30) {
        throw new https_1.HttpsError("invalid-argument", "Duration must be between 1 and 30 days.");
    }
    ...
    return await db.runTransaction(async (transaction) => {
        ...
        const walletSnap = await transaction.get(walletRef);
        if (!walletSnap.exists) {
            throw new https_1.HttpsError("failed-precondition", "User wallet not found.");
        }
        const wallet = getOrCreateWallet(walletSnap, userId);
        if (wallet.chapaBalance < totalCost) {
            throw new https_1.HttpsError("failed-precondition", `Insufficient funds to boost post. Cost: ${totalCost.toFixed(2)} ETB, Balance: ${wallet.chapaBalance.toFixed(2)} ETB.`);
        }
        // 2. Verify post existence
        const postSnap = await transaction.get(postRef);
        if (!postSnap.exists) {
            throw new https_1.HttpsError("not-found", "Target post does not exist.");
        }
        const now = admin.firestore.Timestamp.now();
 ...
 */
exports.createAdCampaign = (0, https_1.onCall)({ cors: true }, async (request) => {
    const userId = request.auth?.uid;
    if (!userId) {
        throw new https_1.HttpsError("unauthenticated", "Authentication required to create ad campaign.");
    }
    const { campaignName, objective, dailyBudgetETB, durationInDays, adHeadline, primaryText, callToAction, destinationUrl, } = request.data;
    if (!campaignName || !objective || !adHeadline || !primaryText) {
        throw new https_1.HttpsError("invalid-argument", "All campaign creative fields and objective are required.");
    }
    const dailyBudget = roundCurrency(Number(dailyBudgetETB) || 0);
    const duration = Math.floor(Number(durationInDays) || 0);
    if (dailyBudget < 25) {
        throw new https_1.HttpsError("invalid-argument", "Minimum daily budget for Ads Manager is 25.00 ETB.");
    }
    if (duration < 1 || duration > 90) {
        throw new https_1.HttpsError("invalid-argument", "Campaign duration must be between 1 and 90 days.");
    }
    ...
    return await db.runTransaction(async (transaction) => {
        ...
        if (wallet.chapaBalance < totalBudget) {
            throw new https_1.HttpsError("failed-precondition", `Insufficient funds for ad campaign. Budget required: ${totalBudget.toFixed(2)} ETB, Balance: ${wallet.chapaBalance.toFixed(2)} ETB.`);
        }
        const now = admin.firestore.Timestamp.now();
 ...
 */
exports.depositWebhook = (0, https_1.onRequest)(async (req, res) => {
    if (req.method !== "POST") {
